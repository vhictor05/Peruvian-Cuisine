...
API_URL = "http://localhost:8000/api/v1/disco/eventos"
...
class EventosVista:
        ...
        def task():
            try:
                if not hasattr(self, 'fecha_hora_seleccionada') or not self.fecha_hora_seleccionada:
                    self.parent.after(0, lambda: messagebox.showerror("Error", "Por favor seleccione una fecha y hora para el evento"))
                    return
                evento_data = {
                    "nombre": self.evento_nombre.get(),
                    "descripcion": self.evento_descripcion.get(),
                    "fecha": self.fecha_hora_seleccionada.strftime("%Y-%m-%d %H:%M:%S"),
                    "precio_entrada": float(self.evento_precio.get()),
                    "aforo_maximo": int(self.evento_aforo.get())
                }
                r = requests.post(API_URL, json=evento_data)
                if r.status_code == 201:
                    self.parent.after(0, lambda: [messagebox.showinfo("Éxito", "Evento registrado"), self.actualizar_lista_eventos(), self.limpiar_campos()])
                else:
                    self.parent.after(0, lambda: messagebox.showerror("Error", f"Error al registrar evento: {r.text}"))
            except ValueError as ve:
                self.parent.after(0, lambda: messagebox.showerror("Error", "Por favor ingrese valores numéricos válidos para precio y aforo"))
            except Exception as e:
    ...
    def editar_evento(self):
        def task():
            ...
            if not selected_item:
                self.parent.after(0, lambda: messagebox.showwarning("Advertencia", "Seleccione un evento para editar"))
                return
            try:
                ...
                r = requests.put(f"{API_URL}/{evento_id}", json=nuevos_datos)
                if r.status_code == 200:
                    self.parent.after(0, lambda: [messagebox.showinfo("Éxito", "Evento actualizado correctamente"), self.actualizar_lista_eventos(), self.limpiar_campos()])
                else:
                    self.parent.after(0, lambda: messagebox.showerror("Error", f"No se pudo actualizar el evento: {r.text}"))
            except ValueError:
                self.parent.after(0, lambda: messagebox.showerror("Error", "Por favor ingrese valores numéricos válidos para precio y aforo"))
            except Exception as e:
                self.parent.after(0, lambda: messagebox.showerror("Error", f"Error al editar evento: {str(e)}"))
        threading.Thread(target=task).start()
    ...
    def eliminar_evento(self):
        def task():
            selected_item = self.evento_tree.selection()
            if not selected_item:
                self.parent.after(0, lambda: messagebox.showwarning("Advertencia", "Seleccione un evento para eliminar"))
                return
            evento_id = self.evento_tree.item(selected_item[0], "values")[0]
            ...
            if confirmacion:
                try:
                    r = requests.delete(f"{API_URL}/{evento_id}")
                    if r.status_code == 204:
                        self.parent.after(0, lambda: [messagebox.showinfo("Éxito", "Evento eliminado correctamente"), self.actualizar_lista_eventos(), self.limpiar_campos()])
                    else:
                        self.parent.after(0, lambda: messagebox.showerror("Error", f"No se pudo eliminar el evento: {r.text}"))
                except Exception as e:
                    self.parent.after(0, lambda: messagebox.showerror("Error", f"No se pudo eliminar el evento: {str(e)}"))
        threading.Thread(target=task).start()
    ...
    def cargar_evento_seleccionado(self, event):
        def task():
            ...
            evento_id = self.evento_tree.item(selected_item[0], "values")[0]
            try:
                r = requests.get(f"{API_URL}/{evento_id}")
                if r.status_code == 200:
                    evento = r.json()
                    def update_fields():
                        self.limpiar_campos()
                        self.evento_nombre.insert(0, evento['nombre'])
                        self.evento_descripcion.insert(0, evento.get('descripcion', ""))
                        self.evento_precio.insert(0, str(evento['precio_entrada']))
                        self.evento_aforo.insert(0, str(evento['aforo_maximo']))
                        self.fecha_evento_entry.insert(0, evento['fecha'])
                        self.fecha_hora_seleccionada = datetime.strptime(evento['fecha'], "%Y-%m-%d %H:%M:%S")
                    self.parent.after(0, update_fields)
            except Exception as e:
                self.parent.after(0, lambda: messagebox.showerror("Error", f"No se pudo cargar el evento: {str(e)}"))
        threading.Thread(target=task).start()
